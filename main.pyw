        ...
        if self.text.get() == "" or self.text.get() == "e.g C:/Users/User/Pictures/Saved Pictures/cataract.png": #check if no file is selected
            ...
            if myObj.CDetector() <= 70: #70 is the threshold
                ...
                if myObj2.BDetector() <= 200: # write to the file the second part
                    handler.write(str(myObj2.BDetector()) + "%" + " | Insufficent evidence of blood presence" "\n")
                else: # --"-- if the ‰ is bigger than the threshold
                    handler.write(str(myObj2.BDetector()) + "%" + " | Possible Blood presence" "\n")

                #print("Insufficent evidence of cataract") //ignore

            elif myObj2.BDetector() > 200:
                if myObj.CDetector() > 70:
                    handler.write(self.newtext[-1] + ", " + str(myObj.CDetector()) + "%" + " | Cataract detected" + ", " + str(myObj2.BDetector()) + "%" + " | Possible blood presence" "\n")
                else:
                    handler.write(self.newtext[-1] + ", " + str(myObj.CDetector()) + "%" + " | Insufficent evidence of cataract" + ", "+ str(myObj2.BDetector()) + "%" + " | Possible blood presence" "\n")
            self.CreateMsg.pack()
